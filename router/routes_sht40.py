...
from models import db, SHT40
from datetime import datetime, timedelta

bp_sht40 = Blueprint('sht40', __name__)
@bp_sht40.route('/sht40', methods=['POST'])
def receive_sht40_data():
    data = request.json
    if not data:
...
@bp_sht40.route('/dev_sht40/device_ids', methods=['GET'])
def get_device_ids():
    # 获取所有唯一的 device_id
    device_ids = db.session.query(SHT40.device_id).distinct().all()
...
@bp_sht40.route('/dev_sht40/data/<device_id>', methods=['GET'])
def get_data_by_device_id(device_id):
    ...
    query = db.session.query(SHT40).filter_by(device_id=device_id)

    # 如果提供了时间范围，则按时间范围过滤
    if start_time:
        try:
            start_time = datetime.fromisoformat(start_time)
            query = query.filter(SHT40.timestamp >= start_time)
        except ValueError:
    ...
    if end_time:
        try:
            ...
            query = query.filter(SHT40.timestamp <= end_time)
        except ValueError:
